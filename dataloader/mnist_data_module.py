...
class ScaleMNISTDataModule(pl.LightningDataModule):
  ...
  def setup(self, stage=None):
    if stage == 'fit' or stage is None:
      ...
      dataset_train = MNIST(self.data_dir, train=True, download=True, transform= transforms.ToTensor())
      dataset_test = MNIST(self.data_dir, train=False, download=True, transform= transforms.ToTensor())
