...
class ScaleSTL(Dataset):
  def __init__(self,root_data, scales, batch_size, data_resize_mode, data_downscale_mode, transform=None):
    ...
    self.normalize =  transforms.Normalize((0.4467, 0.4398, 0.4066), (0.2603, 0.2566, 0.2713))
  ...
  def __getitem__(self, index: int):
    ...
    if self.data_downscale_mode == "ideal":
      img = resample(img[None,:,:,:],(self.scale_range[i],self.scale_range[i]), complex= False, skip_nyq=True)
    ...
    if self.data_resize_mode == "pad":
      ...
      l = self.max_size - img.shape[-2]
      t = self.max_size - img.shape[-1]
      img = pad(img, pad = [l//2, l - l//2, t//2, t - t//2], mode="reflect")
    elif self.data_resize_mode == "replicate":
      l = self.max_size - img.shape[-2]
      t = self.max_size - img.shape[-1]
      img = pad(img, pad = [l//2, l - l//2, t//2, t - t//2], mode="replicate")
    elif self.data_resize_mode == "circular":
      l = self.max_size - img.shape[-2]
      t = self.max_size - img.shape[-1]
      img = pad(img, pad = [l//2, l - l//2, t//2, t - t//2], mode='circular')
