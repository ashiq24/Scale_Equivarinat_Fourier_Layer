...
from utils.core_utils import AddUniformNoise, AddCircularShift, Cutout
def None_func():
  return None
augment_map = {"add_noise":AddUniformNoise, "circular_shift": AddCircularShift, "None": None_func}
...
class ScaleSTLDataModule(pl.LightningDataModule):
  ...
  def setup(self, stage=None):
    ...
    if stage == 'fit' or stage is None:
      ...
      dataset_train = STL10(root=self.data_dir,split='train',download=True, transform= transforms.ToTensor())
      dataset_test = STL10(root=self.data_dir,split='test',download=True, transform= transforms.ToTensor())
