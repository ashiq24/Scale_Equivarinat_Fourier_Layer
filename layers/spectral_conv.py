...
class SpectralConv2dCircular(nn.Module):
  ...
  def forward(self, x_ft, pool = False):
    ...
    out_ft = torch.zeros(batchsize, self.out_channel,  dim1, dim2 , dtype= x_ft.type, device=x_ft.device)
    out_ft[:, :, :effective_h_modes1, :effective_h_modes2 ] = \
        compl_mul2d(x_ft[:, :, :effective_h_modes1, :effective_h_modes2 ], self.W[:,:,:effective_h_modes1, :effective_h_modes2 ])
...
class SpectralConv2dLocalized(nn.Module):
  def __init__(self, in_channel, out_channel, global_modes, local_modes, device = None, dtype = torch.cfloat):
    ...
    self.L = nn.Parameter(torch.empty( self.in_channel, self.out_channel,local_modes, self.local_modes , **factory_kwargs))
    self.register_buffer("spectral_decay", get_pweights(local_modes,local_modes))
    self.register_buffer("mat",get_mat(self.global_modes, self.L.shape[-2]).to(self.L.device, dtype = self.L.dtype))
  ...
  def reset_parameter(self):
    scale = (1/self.in_channel)**0.5 #self.global_modes/(2*self.local_h_modes)
  ...
  def forward(self, x_ft, pool = False):
    ...
    out_ft[:, :, :effective_h_modes1+s, :effective_h_modes2+s ] = \
        compl_mul2d(x_ft[:, :, :effective_h_modes1+s, :effective_h_modes2+s ], W[:,:,:effective_h_modes1+s, :effective_h_modes2+s ])

    out_ft[:, :, -effective_h_modes1:, :effective_h_modes2+s] = \
        compl_mul2d(x_ft[:, :, -effective_h_modes1:, :effective_h_modes2+s], W[:,:,-effective_h_modes1:, :effective_h_modes2+s])

    out_ft[:, :, :effective_h_modes1+s, -effective_h_modes2: ] = \
        compl_mul2d(x_ft[:, :, :effective_h_modes1+s, -effective_h_modes2: ], W[:,:,:effective_h_modes1+s, -effective_h_modes2: ])

    out_ft[:, :, -effective_h_modes1:, -effective_h_modes2:] = \
        compl_mul2d(x_ft[:, :, -effective_h_modes1:, -effective_h_modes2:], W[:,:,-effective_h_modes1:, -effective_h_modes2:])
...
class SpectralConvLocalized1d(nn.Module):
  def __init__(self, in_channel, out_channel, global_modes, local_modes, device = None, dtype = torch.cfloat):
    ...
    self.local_modes = local_modes
    self.global_modes = int(global_modes)
    self.L = nn.Parameter(torch.empty( self.in_channel, self.out_channel,local_modes , **factory_kwargs))
    self.register_buffer("spectral_decay", get_pweights(local_modes,local_modes)[0,:])
    self.reset_parameter()
  ...
  def get_filters(self,):
    mat = get_mat(self.global_modes, self.L.shape[-1]).to(self.L.device, dtype = self.L.dtype)
  ...
  def forward(self, x_ft, pool = False):
    ...
    out_ft = torch.zeros(batchsize, self.out_channel, dim , dtype=x_ft.dtype, device=self.L.device)

    out_ft[:, :, :effective_h_modes1+s] = \
        compl_mul1d(x_ft[:, :, :effective_h_modes1+s], W[:,:,:effective_h_modes1+s])

    out_ft[:, :, -effective_h_modes1:] = \
        compl_mul1d(x_ft[:, :, -effective_h_modes1:], W[:,:,-effective_h_modes1:])
  ...
  def forward(self,x):
    ...
    for i in range(self.number):
      for j in range(self.number):
        ...
        mr = resample(m[None,None,:,:], num = (x.shape[-2], x.shape[-1]), complex= False)[0,0,:,:]
        x_t = x * mr[None,None,:,:]
        if t is None:
          t = x_t
        else:
