...
def get_block2d(t, modes):
    ...
    if len(t.shape) == 3:
        new_mean = torch.zeros(
            t.shape[0], h_modes*2 + s, 2*h_modes + s, dtype=t.dtype, device=t.device)
    else:
        new_mean = torch.zeros(
            t.shape[0], t.shape[1], h_modes*2 + s, 2*h_modes + s, dtype=t.dtype, device=t.device)
...
def get_block1d(t, modes):
    ...
    if len(t.shape) == 2:
        new_mean = torch.zeros(
            t.shape[0], h_modes*2+s, dtype=t.dtype, device=t.device)
    else:
        new_mean = torch.zeros(
            t.shape[0], t.shape[1], h_modes*2+s, dtype=t.dtype, device=t.device)
...
def enblock2d(t, modes):
    ...
    if len(t.shape) == 3:
        new_mean = torch.zeros(
            t.shape[0], modes, modes, dtype=t.dtype, device=t.device)
    else:
        new_mean = torch.zeros(
            t.shape[0], t.shape[1], modes, modes, dtype=t.dtype, device=t.device)
...
def enblock1d(t, modes):
    ...
    if len(t.shape) == 2:
        new_mean = torch.zeros(
            t.shape[0], modes, dtype=t.dtype, device=t.device)
    else:
        new_mean = torch.zeros(
            t.shape[0], t.shape[1], modes, dtype=t.dtype, device=t.device)
...
class ComplexBatchNorm2dSim(Module):
    def __init__(self, num_features, modes, eps=1e-5, momentum=0.1, affine=True,
        ...
        if self.track_running_stats:
            self.register_buffer('running_mean', torch.zeros(
                num_features, modes, modes, dtype=torch.complex64))
            self.register_buffer('running_covar_real',
                                 torch.ones(num_features, modes, modes))
            self.register_buffer('running_covar_imag',
                                 torch.ones(num_features, modes, modes))
    ...
    def forward(self, input, modes=None):
        ...
        if self.training and self.track_running_stats:
            # update running mean
            mean_t = exponential_average_factor * mean + \
                (1 - exponential_average_factor) * \
                get_block2d(self.running_mean, input.shape[-1])
            with torch.no_grad():

                h_modes = input.shape[-1]//2
                s = input.shape[-1] % 2
                self.running_mean[:, :h_modes+s, :h_modes +
                                  s] = mean_t[:, :h_modes+s, :h_modes+s]
                self.running_mean[:, -h_modes:, :h_modes +
                                  s] = mean_t[:, -h_modes:, :h_modes+s]
                self.running_mean[:, :h_modes+s, -
                                  h_modes:] = mean_t[:, :h_modes+s, -h_modes:]
                self.running_mean[:, -h_modes:, -
                                  h_modes:] = mean_t[:, -h_modes:, -h_modes:]
        ...
        if self.training or (not self.training and not self.track_running_stats):
            ...
            Crr = get_block2d(self.running_covar_real,
                              input.shape[-1]) + self.eps
            Cii = get_block2d(self.running_covar_imag,
                              input.shape[-1])+self.eps

        if self.training and self.track_running_stats:
            running_covar_real_temp = exponential_average_factor * Crr + \
                (1 - exponential_average_factor) * \
                get_block2d(self.running_covar_real, input.shape[-1])

            running_covar_imag_temp = exponential_average_factor * Cii + \
                (1 - exponential_average_factor) * \
                get_block2d(self.running_covar_imag, input.shape[-1])
            with torch.no_grad():

                h_modes = input.shape[-1]//2
                s = input.shape[-1] % 2
                self.running_covar_real[:, :h_modes+s, :h_modes +
                                        s] = running_covar_real_temp[:, :h_modes+s, :h_modes+s]
                self.running_covar_real[:, -h_modes:, :h_modes +
                                        s] = running_covar_real_temp[:, -h_modes:, :h_modes+s]
                self.running_covar_real[:, :h_modes+s, -
                                        h_modes:] = running_covar_real_temp[:, :h_modes+s, -h_modes:]
                self.running_covar_real[:, -h_modes:, -
                                        h_modes:] = running_covar_real_temp[:, -h_modes:, -h_modes:]

                self.running_covar_imag[:, :h_modes+s, :h_modes +
                                        s] = running_covar_imag_temp[:, :h_modes+s, :h_modes+s]
                self.running_covar_imag[:, -h_modes:, :h_modes +
                                        s] = running_covar_imag_temp[:, -h_modes:, :h_modes+s]
                self.running_covar_imag[:, :h_modes+s, -
                                        h_modes:] = running_covar_imag_temp[:, :h_modes+s, -h_modes:]
                self.running_covar_imag[:, -h_modes:, -
                                        h_modes:] = running_covar_imag_temp[:, -h_modes:, -h_modes:]
...
class ComplexBatchNorm1dSim(Module):
    def __init__(self, num_features, modes, eps=1e-5, momentum=0.1, affine=True,
        ...
        if self.track_running_stats:
            self.register_buffer('running_mean', torch.zeros(
                num_features, modes, dtype=torch.complex64))
            self.register_buffer('running_covar_real',
                                 torch.ones(num_features, modes))
            self.register_buffer('running_covar_imag',
                                 torch.ones(num_features, modes))
            self.register_buffer('num_batches_tracked',
                                 torch.tensor(0, dtype=torch.long))
    ...
    def forward(self, input, modes=None):
        ...
        if self.training and self.track_running_stats:
            ...
            mean_t = exponential_average_factor * mean + \
                (1 - exponential_average_factor) * \
                get_block1d(self.running_mean, input.shape[-1])
            with torch.no_grad():
        ...
        if self.training or (not self.training and not self.track_running_stats):
            ...
            Crr = get_block1d(self.running_covar_real,
                              input.shape[-1]) + self.eps
            Cii = get_block1d(self.running_covar_imag,
                              input.shape[-1])+self.eps

        if self.training and self.track_running_stats:
            running_covar_real_temp = exponential_average_factor * Crr + \
                (1 - exponential_average_factor) * \
                get_block1d(self.running_covar_real, input.shape[-1])

            running_covar_imag_temp = exponential_average_factor * Cii + \
                (1 - exponential_average_factor) * \
                get_block1d(self.running_covar_imag, input.shape[-1])
            with torch.no_grad():

                h_modes = input.shape[-1]//2
                s = input.shape[-1] % 2
                self.running_covar_real[:, :h_modes+s,
                                        ] = running_covar_real_temp[:, :h_modes+s]
                self.running_covar_real[:, -
                                        h_modes:] = running_covar_real_temp[:, -h_modes:]

                self.running_covar_imag[:, :h_modes +
                                        s] = running_covar_imag_temp[:, :h_modes+s]
                self.running_covar_imag[:, -
                                        h_modes:] = running_covar_imag_temp[:, -h_modes:]
            Crr = running_covar_real_temp
            Cii = running_covar_imag_temp
