...
from utils.core_utils import get_mixer_mask, ComplexGELU, ComplexSELU, get_pweights, ComplexLeakyRELU, ComplexTanh, ComplexRELU, get_pweights_1D
...
class SpectralMixer_1D(nn.Module):
  def __init__(self, num_channels, mode_size, mixer_band =-1, device= None, dtype = torch.cfloat):
    ...
    self.W1 = nn.Parameter(torch.empty(num_channels, self.mode_size, self.mode_size, **factory_kwargs))
    self.bias1 = nn.Parameter(torch.empty(num_channels,self.mode_size, **factory_kwargs))
    self.mixer_band = mixer_band
    
    self.register_buffer("spectral_decay", get_pweights(mode_size,mode_size))
  ...
  def forward(self, x, in_modes1=None):
    ...
    s = in_modes1%2
    temp = torch.zeros(x.shape[0], x.shape[1],  h_modes1*2 +s,dtype= x.dtype, device=x.device)

    eW1 = torch.zeros(x.shape[1], h_modes1*2+s, 2*h_modes1+s , dtype=x.dtype, device=x.device)
    bias1 = torch.zeros(x.shape[1],  h_modes1*2+s, dtype=x.dtype, device=x.device)
    ...
    mask = get_mixer_mask(h_modes1*2+s, dtype=x.dtype, band_width= self.mixer_band).to(x.device)

    temp = ((eW1 * mask[None,:,:])[None,:,:,:] @ temp[...,None])[:,:,:,0] + bias1[None,:,:]
...
class SpectralMixer_2D(nn.Module):
  def __init__(self, num_channels, mode_size, mixer_band =-1, device= None, dtype = torch.cfloat):
    ...
    self.mode_size = mode_size
    self.W1 = nn.Parameter(torch.empty(num_channels, self.mode_size, self.mode_size, **factory_kwargs))
    self.W2 = nn.Parameter(torch.empty(num_channels, self.mode_size, self.mode_size, **factory_kwargs))
    self.bias1 = nn.Parameter(torch.empty(num_channels,self.mode_size, self.mode_size, **factory_kwargs))
    self.mixer_band = mixer_band
  ...
  def forward(self, x, in_modes1, in_modes2):
    ...
    eW1 = torch.zeros(x.shape[1], h_modes1*2+s, 2*h_modes2+s , dtype=x.dtype, device=x.device)
    eW2 = torch.zeros(x.shape[1],  h_modes1*2+s, 2*h_modes2+s, dtype=x.dtype, device=x.device)
    bias1 = torch.zeros(x.shape[1],  h_modes1*2+s, 2*h_modes2+s, dtype=x.dtype, device=x.device)
    ...
    bias1[ :, :h_modes1+s, :h_modes2+s ] = self.bias1[:,:h_modes1+s, :h_modes2+s ]
    bias1[ :, -h_modes1:, :h_modes2+s] = self.bias1[:,-h_modes1:, :h_modes2+s]
    ...
    temp = (torch.transpose((eW2*mask[None,:,:])[None,:,:,:] @ torch.transpose(temp,-2,-1), -2,-1)) 
...
class SpectralMixer_2D_shifteq(nn.Module):
  ...
  def forward(self, x, in_modes1, in_modes2):
    ...
    eW1 = torch.zeros(x.shape[1], h_modes1*2+s, 2*h_modes2+s , dtype=self.W1.dtype, device=x.device)
    eW2 = torch.zeros(x.shape[1],  h_modes1*2+s, 2*h_modes2+s, dtype=self.W2.dtype, device=x.device)
    bias1 = torch.zeros(x.shape[1],  h_modes1*2+s, 2*h_modes2+s, dtype=self.bias1.dtype, device=x.device)
    ...
    mask = get_mixer_mask(h_modes1*2+s,dtype=self.W1.dtype, band_width = self.mixer_band).to(x.device)
