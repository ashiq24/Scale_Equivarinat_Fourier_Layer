...
class spectralMlp(nn.Module):
  ...
  def __init__(self, modes, num_feature, out_channel, non_linear,mixer_band =-1, device = None, dtype = torch.cfloat):
    ...
    self.W = nn.Parameter(torch.empty(out_channel, modes, num_feature,**factory_kwargs))
    self.register_buffer("mask", get_mixer_mask(modes,factory_kwargs['dtype'], band_width= mixer_band))
  ...
  def forward(self,x):
    ...
    '''
    out = x[:,None,None,:] @ torch.transpose(self.W * self.mask.repeat(1,self.num_feature//self.modes)[None,:,:] , -2, -1)
    out = out 
