...
class ComplexConv1d(Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=0,
        ...
        self.conv_r = Conv1d(in_channels, out_channels,
                             kernel_size, stride, padding, dilation, groups, bias)
        self.conv_i = Conv1d(in_channels, out_channels,
                             kernel_size, stride, padding, dilation, groups, bias)
...
class ComplexBatchNorm1d(_ComplexBatchNorm):
    ...
    def forward(self, input):
        ...
        if self.training or (not self.training and not self.track_running_stats):
            ...
            n = input.numel() / input.size(1)
            Crr = input.real.var(dim=0, correction=0)+self.eps
            Cii = input.imag.var(dim=0, correction=0)+self.eps
            Cri = (input.real.mul(input.imag)).mean(dim=0)  # +self.eps
        else:
            Crr = self.running_covar[:, 0]+self.eps
            Cii = self.running_covar[:, 1]+self.eps
            Cri = self.running_covar[:, 2]  # +self.eps
        ...
        if self.training and self.track_running_stats:
            with torch.no_grad():
                self.running_covar[:, 0] = exponential_average_factor * Crr \
                    + (1 - exponential_average_factor) * \
                    self.running_covar[:, 0]

                self.running_covar[:, 1] = exponential_average_factor * Cii \
                    + (1 - exponential_average_factor) * \
                    self.running_covar[:, 1]

                self.running_covar[:, 2] = exponential_average_factor * Cri \
                    + (1 - exponential_average_factor) * \
                    self.running_covar[:, 2]
            Crr = self.running_covar[:, 0]+self.eps
            Cii = self.running_covar[:, 1]+self.eps
        ...
        if self.track_running_stats:
            input = input + mean[None, ...] - self.running_mean[None, ...]

        input = (Rrr[None, :] * torch.real(input)+Rri[None, :]*torch.imag(input)) \
            + 1.0j*(Rii[None, :]*torch.imag(input) +
                    Rri[None, :] * torch.real(input))
...
class _ComplexBatchNormModified(Module):

    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True,
                 track_running_stats=True):
        super(_ComplexBatchNormModified, self).__init__()
        self.num_features = num_features
        self.eps = eps
        self.momentum = momentum
        self.affine = affine
        ...
        if self.track_running_stats:
            self.register_buffer('running_mean', torch.zeros(
                num_features, dtype=torch.complex64))
            self.register_buffer('running_covar', torch.zeros(num_features, 2))
            self.running_covar[:, 0] = 1
            self.running_covar[:, 1] = 1
            self.register_buffer('num_batches_tracked',
                                 torch.tensor(0, dtype=torch.long))
        else:
            self.register_parameter('running_mean', None)
    ...
    def reset_parameters(self):
        self.reset_running_stats()
        if self.affine:
            init.constant_(self.weight[:, :2], 1)
            init.zeros_(self.bias)
