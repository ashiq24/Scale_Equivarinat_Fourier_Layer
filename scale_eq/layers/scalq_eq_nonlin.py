...
from scale_eq.utils.core_utils import resample, resample_1d, down_sample_keepsize
...
class scaleEqNonlin(nn.Module):
    def __init__(self, non_lin, base_res, normalization=None, channels=None, max_res=28, increment=1) -> None:
        ...
        print("Making Non_lin layer, Max res",
              max_res, " Increment", self.increment)
        if normalization is not None:
            if normalization == 'instance':
                self.norm = nn.ModuleDict()
                for res in range(self.base_res, max_res+self.increment, self.increment):
                    self.norm[str(res)] = nn.InstanceNorm2d(
    ...
    def forward(self, x_ft):
        ...
        x_out = torch.zeros(x.shape[0], x.shape[1], x.shape[2],
                            x.shape[3]//2 + 1, dtype=x_ft.dtype).to(x_ft.device)
        ...
        x_out[..., :self.base_res//2, :self.base_res//2 +
              1] = xbase_rft[..., :self.base_res//2, :self.base_res//2 + 1]
        x_out[..., -(self.base_res//2):, :self.base_res//2 +
              1] = xbase_rft[..., -(self.base_res//2):, :self.base_res//2 + 1]
        ...
        if self.increment == 1:
            for res in range(self.base_res+1, x.shape[-1]+1):
                # resample(x, [res,res], complex= False) #
                x_res = down_sample_keepsize(X, [res, res], res)
                if self.norm is not None:
                ...
                if s == 1:
                    x_out[..., res//2, :res//2 +
                          1] = x_res_rft[..., res//2, :res//2+1]
                    x_out[..., -(res//2), :res//2 +
                          1] = x_res_rft[..., -(res//2), :res//2+1]
                    x_out[..., :res//2+1, res //
                          2] = x_res_rft[..., :res//2+1, res//2]
                    x_out[..., -(res//2):, res //
                          2] = x_res_rft[..., -(res//2):, res//2]
                else:
                    x_out[..., -(res//2), :res//2 +
                          1] = x_res_rft[..., -(res//2), :res//2+1]
                    x_out[..., -(res//2):, res //
                          2] = x_res_rft[..., -(res//2):, res//2]
                    x_out[..., :(res//2), res//2] = x_res_rft[...,
                                                              :(res//2), res//2]
        else:
            prev = self.base_res//2 + 1
            for res in range(self.base_res+self.increment, x.shape[-1]+self.increment, self.increment):
                # resample(x, [res,res], complex= False) #
                x_res = down_sample_keepsize(X, [res-1, res-1], res)
                if self.norm is not None:
                    x_res = self.norm[str(res)](x_res)
                x_res = self.non_lin(x_res)
                x_res_rft = torch.fft.rfft2(x_res, norm='forward')
                ...
                x_out[..., prev-1:t_res//2, :t_res//2 +
                      1] = x_res_rft[..., prev-1:t_res//2, :t_res//2+1]
                x_out[..., -(t_res//2):-(prev - 1), :t_res//2 +
                      1] = x_res_rft[..., -(t_res//2):-(prev - 1), :t_res//2+1]
                x_out[..., :t_res//2, prev-1: t_res//2 +
                      1] = x_res_rft[..., :t_res//2, prev-1: t_res//2+1]
                x_out[..., -(t_res//2):, prev-1: t_res//2 +
                      1] = x_res_rft[..., -(t_res//2):, prev-1: t_res//2+1]
        ...
        x_out = torch.fft.fft2(torch.fft.irfft2(
            x_out, s=(x.shape[-2], x.shape[-1]), norm='forward'), norm='forward')
        return x_out
...
class scaleEqNonlinMaxp(nn.Module):
    ...
    def forward(self, x_ft):
        ...
        x_out = torch.zeros(x.shape[0], x.shape[1], out_res,
                            out_res//2 + 1, dtype=x_ft.dtype).to(x_ft.device)
        ...
        if self.norm is not None:
            xbase = self.norm[str(self.base_res)](xbase)
        xbase = self.non_lin(xbase)
        xbase = down_sample_keepsize(xbase, [math.ceil(
            self.base_res/self.pool_window), math.ceil(self.base_res/self.pool_window)], self.base_res, isfreq=False)
        xbase = F.max_pool2d(xbase, self.pool_window, ceil_mode=True)

        res = xbase.shape[-1]
        xbase_rft = torch.fft.rfft2(xbase, norm='forward')
        x_out[..., :res//2, :res//2 + 1] = xbase_rft[..., :res//2, :res//2 + 1]
        x_out[..., -(res//2):, :res//2 + 1] = xbase_rft[..., -
                                                        (res//2):, :res//2 + 1]
        ...
        if self.increment == 1:
            for res in range(self.base_res+self.pool_window, x.shape[-1]+self.pool_window, self.pool_window):
                ...
                x_res = self.non_lin(x_res)
                x_res = down_sample_keepsize(x_res, [math.ceil(
                    res/self.pool_window), math.ceil(res/self.pool_window)], res, isfreq=False)
                ...
                s = c_res % 2
                if s == 1:
                    x_out[..., c_res//2, :c_res//2 +
                          1] = x_res_rft[..., c_res//2, :c_res//2+1]
                    x_out[..., -(c_res//2), :c_res//2 +
                          1] = x_res_rft[..., -(c_res//2), :c_res//2+1]
                    x_out[..., :c_res//2+1, c_res //
                          2] = x_res_rft[..., :c_res//2+1, c_res//2]
                    x_out[..., -(c_res//2):, c_res //
                          2] = x_res_rft[..., -(c_res//2):, c_res//2]
        ...
        x_out = torch.fft.fft2(torch.fft.irfft2(
            x_out, s=(out_res, out_res), norm='forward'), norm='forward')
        return x_out
...
class scaleEqNonlin1d(nn.Module):
    ...
    def forward(self, x_ft):
        ...
        x_out = torch.zeros(
            x.shape[0], x.shape[1], x.shape[2]//2 + 1, dtype=x_ft.dtype).to(x_ft.device)
        ...
        for res in range(self.base_res+1, x.shape[-1]+1):
            ...
            if s == 1:
                ...
                x_out[..., -(res//2):] = x_res_rft[..., -(res//2):]
                x_out[..., :(res//2)] = x_res_rft[..., :(res//2)]
        x_out = torch.fft.fft(torch.fft.irfft(
            x_out, n=x.shape[-1], norm='forward'), norm='forward')
        return x_out
