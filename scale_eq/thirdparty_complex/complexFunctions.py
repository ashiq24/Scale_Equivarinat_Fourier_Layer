...
def _retrieve_elements_from_indices(tensor, indices):
    ...
    output = flattened_tensor.gather(
        dim=-1, index=indices.flatten(start_dim=-2)).view_as(indices)
    return output
...
def complex_max_pool2d(input, kernel_size, stride=None, padding=0,
    ...
    # angle = torch.atan2(input.imag,input.real)
    input_uni = input.clone()
    unit = input_uni/(input_uni.abs() + 1e-5)
    # get only the phase values selected by max pool
    # angle = _retrieve_elements_from_indices(unit, indices)
    return absolute_value * _retrieve_elements_from_indices(unit, indices)
