...
from scale_eq.utils.core_utils import resample, get_stack, get_pooled, get_stack_differential
from scale_eq.layers.spectral_conv import SpectralConv2dCircular, SpectralConv2dLocalized, softPaching
from scale_eq.layers.spectral_mixer import SpectralMixer_2D, SpectralMixer_2D_shifteq
from scale_eq.layers.spectral_mlp import spectralMlp
from scale_eq.layers.scalq_eq_nonlin import scaleEqNonlin, scaleEqNonlinMaxp
...
from scale_eq.thirdparty_complex.complexLayers import ComplexConv2d, ComplexMaxPool2d
...
class FourierSeqStl(AbstractBaseClassifierModel):
    def __init__(self, in_channel, learning_rate, weight_decay, C1=32, C2=64, C3=128, C4=128, FC1=200, dropout_fc1=0.0,
        ...
        self.PN = scaleEqNonlin(activation_con, self.base_res, normalizer,
                                C1, max_res=self.keep_size, increment=self.increment)
        ...
        self.NL0_1 = scaleEqNonlin(activation_con, self.base_res, normalizer,
                                   C2, max_res=self.keep_size, increment=self.increment)
        self.NL0_2 = scaleEqNonlinMaxp(
            activation_con, self.base_res, normalizer, C2, max_res=self.keep_size, increment=self.increment)
        ...
        self.NL1_1 = scaleEqNonlin(activation_con, math.ceil(
            self.base_res/2), normalizer, C3, max_res=math.ceil(self.keep_size/2), increment=self.increment//2)
        self.NL1_2 = scaleEqNonlinMaxp(activation_con, math.ceil(
            self.base_res/2), normalizer, C3, max_res=math.ceil(self.keep_size/2), increment=self.increment//2)
        ...
        self.NL2_1 = scaleEqNonlin(activation_con, math.ceil(
            self.base_res/4), normalizer, C4, max_res=math.ceil(self.keep_size/4), increment=self.increment//4)
        self.NL2_2 = scaleEqNonlin(activation_con, math.ceil(
            self.base_res/4), normalizer, C4, max_res=math.ceil(self.keep_size/4), increment=self.increment//4)
    ...
    def forward(self, x):
        ...
        x_ft = torch.fft.fft2(x, norm='forward').to(
            self.device, dtype=self.L0_1.L.dtype)

        x_ft = self.projector(x_ft)
        x_ft = self.PN(x_ft)

        x1 = self.apply_block(x_ft, self.L0_1, self.L0_2,
                              self.NL0_1, self.NL0_2, self.conv0)
        x2 = self.apply_block(x1, self.L1_1, self.L1_2,
                              self.NL1_1, self.NL1_2, self.conv1)
        x3 = self.apply_block(x2, self.L2_1, self.L2_2,
                              self.NL2_1, self.NL2_2, self.conv2)

        fe = get_stack(x3, self.pool_size, math.ceil(
            self.keep_size/4), base_res=self.base_res//4, increment=self.increment//4)
    ...
    def get_loss(self, output, y):
        ...
        for j in range(output.shape[1]):
            if j == 0:
                ...
                loss += c_loss
                if b_loss is None:
                    b_loss = torch.max(c_loss - prev_loss,
                                       torch.tensor(0.0)).to(output.device)
                else:
                    b_loss += torch.max(c_loss - prev_loss,
                                        torch.tensor(0.0)).to(output.device)
