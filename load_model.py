...
# flags.DEFINE_string('data_resize_mode', 'pad', 'Dataset padding mode - None, pad, or resize')
...
# # Model.
# # 'dss_stl', 'ses_stl', 'SI-cov_stl','SS-cov_stl', 'disco_stl'
# #

# flags.DEFINE_string('model', 'disco_stl', 'Model name.')
...
# ''' two layered FNN (Triangular MLP/ Fully Connected Neural Network) follows the Convolutional layer. 
#  Input size of the first layer is determied from "C4" or number of channel in the 
#  The output size of the first layer is determined by "FC1" '''
...
# flags.DEFINE_string('normalizer', 'instance', 'Activation after MLP layers')

# #{'gelu_c': complex_gelu, 'relu_c': complex_relu, 'selu_c':complex_selu, 'tanh_c':complex_tanh, 'tanh_se': complex_tanh_se, 'mod_relu': modRelu,\
# #                  'gelu': gelu, 'relu': relu, 'selu':selu, 'tanh':tanh,}
# '''No option of normalization is kept as only LayerNorm (addpated to support different resolution) seems to work.'''
...
#       name=name,
#       params=params,
#       project_name='Yeh/scale-invariance',
#       version=None,
...
#   model = get_model(FLAGS.model)(learning_rate = FLAGS.learning_rate, weight_decay = FLAGS.weight_decay,)
...
#   #            ".neptune/"+FLAGS.model+" "+ FLAGS.data_downscale_mode+".pt")
#   model.load_state_dict(torch.load(".neptune/"+FLAGS.model+" "+ FLAGS.data_downscale_mode+".pt"))
#   equ_testter = TestEquivarinecError(model)
#   sys.stdout = open(FLAGS.model+'_output.txt','wt')
#   model.cuda()
#   l = equ_testter.get_equivarience_error(dm.test_dataloader(),[i for i in range(49,98,8)], mode=FLAGS.data_downscale_mode, samples= 200)
#   print("Equivarience Error", l)

#   print("...............Test ACC....................")
#   GT = GetAccuracy(model, dm.test_dataloader())
#   print("Accuracty", GT.get_acc())
#   print("Accracy", GT.get_res_acc([i for i in range(49,98,8)]))

#   r = equ_testter.get_resolution_error(dm.test_dataloader(),[i for i in range(49,98,8)], mode='ideal', samples= 200, resize= FLAGS.data_resize_mode)
#   print("Resolution Consitensy Error", r)
