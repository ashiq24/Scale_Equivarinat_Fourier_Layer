...
from scale_eq.dataloader.stl_data_module import ScaleSTLDataModule
import scale_eq.clargs.data_opts
from scale_eq.models.fourier_seq_stl import FourierSeqStl
...
import numpy as np
from scale_eq.utils.core_utils import Cutout
...
flags.DEFINE_string('data_resize_mode', 'None',
                    'Dataset padding mode - None, pad, or resize')
...
flags.DEFINE_integer(
    'C2', 128, 'Width/number of channels in 2nd Convolutional layer')
flags.DEFINE_integer(
    'C3', 256, 'Width/number of channels in 3rd Convolutional layer')
flags.DEFINE_integer(
    'C4', 512, 'Width/number of channels in 4th Convolutional layer')
flags.DEFINE_integer(
...
flags.DEFINE_float('dropout_fc1', 0.00,
...
flags.DEFINE_boolean('autoresume', False,
                     'enables auto-resume from the last checkpoint if the previous run was incomplete')
...
def main(argv):
    ...
    l = equ_testter.get_equivarience_error(dm.test_dataloader(), [i for i in range(
        49, 98, 8)], mode=FLAGS.data_downscale_mode, samples=500)
